...
/**
 * Will remove a service from viacore-node.json
...
/**
 * Will remove the Node.js service and from the viacore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The viacore-node configuration directory
...
function remove(options, done) {
  ...
  var bitcoreConfigPath = path.resolve(configPath, 'viacore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a viacore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to viacore-node.json
