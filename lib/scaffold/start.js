...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and viacore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed viacore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './viacore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in viacore-node services directory
