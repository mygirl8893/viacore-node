...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'viacore-lib': '^' + bitcore.version,
    'viacore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/viacore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the viacore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the viacore-node directory and configuration
