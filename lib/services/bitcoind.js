...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of viacore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of viacore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of viacore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from viacore. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of viacore-node services will start.');
    node._reindex = true;
  }
};
